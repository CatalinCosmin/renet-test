...
fn new_renet_client(local_ip: &String,server_ip: &String) -> RenetClient {
    let server_addr = server_ip.to_owned().parse().unwrap();
...
fn new_renet_server(server_ip: &String) -> RenetServer {
    let server_addr = server_ip.to_owned().parse().unwrap();
...
fn main() {
    ...
    let server_ip = &args[2].to_string();
    let local_ip = &args[3].to_string();
    ...
    if is_host {
        ...
        app.insert_resource(new_renet_server(server_ip));
        app.add_system(server_update_system);
        app.add_system(server_sync_players);
        app.add_system(move_players_system);
    } else {
        app.add_plugin(RenetClientPlugin);
        app.insert_resource(new_renet_client(local_ip, server_ip));
